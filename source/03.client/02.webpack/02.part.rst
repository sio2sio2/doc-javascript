...
permite que se usa tanto el sistema CommonJS_ como el *ES2015*. Al respecto:

+ Use CommonJS_ en el fichero de configuración de :program:`webpack`, ya que es
  el que soporta nativamente NodeJS_.
+ En el código de la aplicación puede usarse tan CommonJS_ como *ES2015*. Usar
  el segundo, tiene la ventaja de que al ser el estándar aprobado, bajo
  determinadas circunstancias (como no importar aplicaciones de terceros\ [#]_
  dentro del propio código), se genera un código directamente usable en
  navegadores modernos.

  En cualquier caso, nuestra intención no es escribir ficheros directamente
  ejecutables por el navegador, así que esta ventaja es muy secundaria.

  .. warning:: Tenga presente que hay una diferencia bastante patente entre usar
     uno y otro sistema. Cuando se usa el de *ES2015*, el código siempre se
     ...
     usando *CommonJS* es necesario especificarlo en el código.
...
.. code-block:: js
   ...
   import $ from "jquery";
   import logo from "images/logo.png";  // Importación de una imagen.
...
* Para **código Javascript** puede usarse el plugin Provide_:
  ...
  *leaflet*\ [#]_. El identificador puede ser la propiedad de un objeto. Por
  ...
  Entiéndase que lo exportado por el módulo cargado es el valor que adquirirá el
  identificador en el código. Ahora bien, si el módulo exporta de esta manera:
  ...
  suponiendo que también lo hayamos importado con el nombre "*a*" en el código,
  porque podríamos haber usado otro.
...
.. _webpack-css-no-externals:

.. warning:: No hay ningún problema en usar el plugin Provide_, en resultados
   ...
   de entrada para incorporar |CSS| en los resultados con dependencias, hace que
   en los resultados sin dependencias, no haya rastro de la referencia al |CSS|
   por lo que en el desarrollo posterior de una aplicación web habría que volver
   a hacer referencia a este |CSS|. Dicho con un ejemplos, si en el desarrollo
   de la aplicación web hacemos:

   .. code-block:: js

      import "leaflet.mipluginmolon/dist/leaflet.mipluginmolon.css"
      import "leaflet.mipluginmolon/dist/leaflet.mipluginmolon.js";

   suponiendo que ese |CSS| y ese *Javascript* no incluyen código de Leaflet_,
   lograremos la carga del código *Javascript*  de Leaflet_, pero no la carga
   de su |CSS|.
...
* El **codigo Javascript** debe cargarse y excluirse, no simplemente dejar de
  ...
  por valores y valores con claves, con la diferencia de que en *externals* un
...
* Para el **resto** de ficheros, la forma de exclusión depende de cómo se
  ...
  - Si la importación se incluyó en el código, la forma de evitar que los
    ...
    .. note:: Tenga presente que si no tenemos ficheros propios (|CSS|, por
       ...
       Tenga presente, no obstante, :ref:`la advertencia antes hecha
       <webpack-css-no-externals>`.
...
Formato
-------
Un segundo aspecto muy importante es la forma en la que el fichero resultante
exporta. Hay
`diversos formatos <porque e://webpack.js.org/guides/author-libraries/>`_, pero
podemos resumir en dos casos nuestras necesidades:

- Si generamos una aplicación final que contiene todos el código necesario,
  no tenemos por qué preocuparnos por la salida, ya que el código *Javascript*
  no tiene que relacionarse con ningún otro. Por tanto:
...
- Si generamos una librería y no una aplicación final, la solución universal
  es generar como fichero resultante :ref:`un módulo UMD <mod-umd>`, ya que de
  esta manera la librería podrá tanto cargarse directamente en el navegador como
  usarse de componente en un desarrollo con NodeJS_.

  Para explicar las posibilidades de esta configuración, lo más apropiado es
  partir de un código fuente de ejemplo:
  ...
  Y empezar a variar la forma en que se exporta. Empecemos así\ [#]_:
  ...
  Al configurar así, podemos exportar como CommonJS_:

  .. code-block:: js

     X = requiere("./index.bundle.js");
     X.PI;  // 3.141592
  ...
  .. code-block:: js
     ...
     import X from "./index.bundle.js";
     X.PI;  // 3.141592

  pero el fichero no exportará nada al usarse directamente en el navegador.
  También es posible exponer no todo lo exportado, sino una parte:
  ...
  Al hacer esto el resputado de la exportación será exclusivamente :code:`PI`:
  ...
  .. note:: Si se pretende exportar la exportación predeterminada (*default*),
     basta con darle el valor *default*:

     .. code-block:: js

        libraryExport: "default"

  Para que el módulo sea usable directamente en un navegador, es necesario
  incluir *library*:
  ...
  Al añadir el atributo, declaramos que en un navegador lo exportado por el
  código esté disponible a través de la variable *X*. En consecuencia si
  cargamos desde el |HTML| así:

  .. code-block:: html

     <script src="index.bundle.js"></script>

  Tendremos disponible :code:`X.PI` y :code:`X.area`. Por su parte, usando
  CommonJS_ y *ES2015* la importación se hará como ya se ha visto. También es
  posible exportar, no a una variable global, sino al atributo de una variable
  global, en principio, preexistente:
  ...
  De esta manera la exportación estará disponible en :code:`L.X`, donde se
  supone que :code:`L` es una variable global.
...
mediante qué cargador debe procesarse. Esto precisamente es lo que se nos
...
:program:`css-loader` procesa el |CSS| que importa desde el código *Javascript*
de *Javascript* o se incluye como punto de entrada. Si se usa este cargador,
hay, sin embargo, otra posibilidad: también procesa los ficheros |CSS| que se
refieren a través de la cláusula :code:`@import`:
...
En este caso, las rutas siguen el criterio general y no de la importación en
código *Javascript* por lo que esa cláusula cargara el fichero que se
encuentre en el mismo directorio que el fichero que se procesa. Para
incoporar |CSS| de terceros, :program:`webpack` incorpora la sintaxis con
virgulilla (:code:`~`). por ejemplo:
...
este código importaría el |CSS| de Leaflet_.

Sea como sea, :program:`css-loader` genera un módulo de *Javascript*, cuya
...
:program:`sass-loader` o :program:`less-loader` u otra cargador semejante.
   ...
   Si optamos por |SASS|, la configuración podría ser esta\ [#]_:
   ...
   .. code-block:: js

      {
         ...
         use: [MiniCssExtractPlugin.loader,
               "css-loader", 
               // Aquí en medio podríamos incluir postcss-loader
               "sass-loader"]
      }
...
- Si se usa :program:`css-loader` para procesar el código |CSS|, las imágenes
  incluidas en sentencias :code:`url()` y las hojas importadas :ref:`a través de
  @import <webpack-@import>`.
...
* *contentBase* indica un directorio del sistema de ficheros que se
  quiere fusionar con el directorio raíz del servidor web. Su valor
...
- Si es ``false``, que es su valor predeterminado al no haber incluido el
  argumento :code:`--open` en :file:`package.json`, no se abre una ventana
...
donde la forma de distinguir una salida de otra es mediante un objeto *env*\
[#]_ que nos encargaremos de desarrollar más adelante. No obstante, dependiendo
de cuál sea el objetivo de nuestro desarrollo podremos prescindir de algunas
salidas:
...
.. [#] La línea del código:
   ...
   define el objeto global que, por defecto, es *windows* y, por tanto, hace la
   aplicación exclusivamente ejecutable en navegadores. Esa es nuestra
   intención, así que podríamos ahorrarnos la línea; pero al fijarlo a *this*,
   el código se hace ejecutable tanto en un navegador como en NodeJS_, con lo
   que podemos hacer pruebas en la propia consola.
